...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.listViewbtn.setGeometry(QtCore.QRect(50, 50, 75, 23))
    ...
    def plot(self):
        ...
        ax1.clear()
        ax1.set_ylim([0,60])
        ax1.plot(a[0,-100:],linewidth=1.4,color=color[0])
        ...
        ax2.clear()
        ax2.set_ylim([0,60])
        ax2.plot(a[1,-100:],linewidth=1.4,color=color[1])
        self.canvas2.draw()

        ax3 = self.fig3.add_subplot(111)
        ax3.clear()
        ax3.set_ylim([0,60])
        ax3.plot(a[2,-100:],linewidth=1.4,color=color[2])
        self.canvas3.draw()

        ax4 = self.fig4.add_subplot(111)
        ax4.clear()
        ax4.set_ylim([0,60])
        ax4.plot(a[3,-100:],linewidth=1.4,color=color[3])
        self.canvas4.draw()

        ax5 = self.fig5.add_subplot(111)
        ax5.clear()
        ax5.set_ylim([0,60])
        ax5.plot(a[4,-100:],linewidth=1.4,color=color[4])
        self.canvas5.draw()

        ax6 = self.fig6.add_subplot(111)
        ax6.clear()
        ax6.set_ylim([0,60])
        ax6.plot(a[cur-1,-100:],linewidth=3.0,color=color[cur-1])
        self.canvas6.draw()
...
##pyshark
def start_Sniffing():
    global sniffingInterface, sniffingTimeOut, a
    """ capture = pyshark.LiveCapture(interface=sniffingInterface)
    capture.set_debug()
    capture.sniff(timeout=sniffingTimeOut) """
    while(1):
